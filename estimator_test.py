...
def model_fn(features, mode):
    train_mode = mode==tf.estimator.ModeKeys.TRAIN
    tf.summary.histogram("final_image_hist", features[0])
    # Define ModelConstructor instance base on the model_name:
    # input_shape and image_type are optional:
    features = base.ModelConstructor(model_name).construct()
    # Define ModelConstructor instance base on the model_name:
    logits = base.Classifier(num_classes).construct(features)
